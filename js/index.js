...
const yearsSecondarySubtitle = document.querySelector('#yearsSecondarySubtitle');
const getOfferList = document.querySelector('.offers__get-list');
...
const whichModelsTrafficPaymentBlock = document.querySelector('#whichModelsTrafficPayment');
...
const projectHiddenText = document.querySelectorAll('.project__hidden_description');
const marqueeContentContainer = document.querySelector('.marquee__content_container');
const maruqeeProjects = document.querySelectorAll('.js-marquee');
const marqueeWrapperProjects = document.querySelectorAll('.js-marquee-wrapper');
if (window.screen.width <= 992) {
  maruqeeProjects[2].classList.add('js-marquee-projects');
  marqueeWrapperProjects[2].classList.add('js-marquee-wrapper-projects');
} else {
  maruqeeProjects[1].classList.add('js-marquee-projects');
  marqueeWrapperProjects[1].classList.add('js-marquee-wrapper-projects');
}

Array.from(projectHiddenText).forEach((hiddenText) => {
  hiddenText.addEventListener('mouseenter', (e) => {
    e.target.nextElementSibling.pause();
  });
  hiddenText.addEventListener('mouseleave', (e) => {
...
let colorIndex = 0;
const colorsArr = ['black', '#2f408e', '#de5722', 'black'];
setInterval(() => {
  colorIndex++;
  wrapper.style.background = colorsArr[colorIndex];
  if (colorIndex >= colorsArr.length) {
    colorIndex = 0;
  }
}, [1000]);
...
intervalId1.toString.callback = function () {
  currentIteration++;
...
intervalId2.toString.callback = function () {
  currentIteration++;
  if (currentIteration >= iterations) {
    stopAnimations();
  }
...
const changeLanguage = (prefix, e) => {
  ...
  getOfferList.textContent = aboutUsObj.getTopOfferList;
  ...
  whichTrafficSourcesBlock.children[1].textContent = whichTrafficSources.answer;
  whichModelsTrafficPaymentBlock.children[0].textContent = whichModelsTrafficPayment.title;
  whichModelsTrafficPaymentBlock.children[1].textContent = whichModelsTrafficPayment.answer;
  availabilityInHouseBlock.children[0].textContent = availabilityInHouse.title;
  availabilityInHouseBlock.children[1].textContent = availabilityInHouse.answer;
...
window.addEventListener('DOMContentLoaded', function () {
  const projectsVideos = document.querySelectorAll('.project__video_source');
  Array.from(projectsVideos).forEach((video) => {
    video.play();
  });
...
window.addEventListener(
  ...
  function () {
    ...
    if (Math.floor(footer.getBoundingClientRect().top) <= arrowBtn.getBoundingClientRect().top) {
